...
Construye una tabla horaria limpia/candidata de variables core GHCNh para
Puerto Rico, 2004–2023.
...
Objetivo
--------
Crear un dataset candidato para comparación futura contra ERA5, aplicando reglas
explícitas y trazables de calidad/unidades.
...
Este script NO debe interpretarse como la versión científica final sin revisar
las tablas de decisiones generadas.
...
----------------------
1. Si el valor raw está dentro de un rango físico amplio, se conserva.
2. Si temperature/dew_point_temperature están fuera de rango, pero raw/10 cae
   dentro del rango, se corrigen con división entre 10.
3. Si station_level_pressure está fuera de rango, pero raw*10 cae dentro del
   rango, se corrige multiplicando por 10.
...
6. Valores fuera de rango que no tienen una corrección justificada se descartan.
...
# Para precipitación usamos suma del intervalo reportado dentro de la hora.
HOURLY_AGG_METHOD = {
    "temperature": "mean",
...
# ---------------------------------------------------------------------------
# Rangos físicos amplios para limpieza candidata
# ---------------------------------------------------------------------------
#
# Son rangos amplios, no climatología fina.
# La intención es remover errores obvios sin eliminar variabilidad física real.
...
def related_code(df: pd.DataFrame, variable: str, suffix: str) -> pd.Series:
    ...
    if col not in df.columns:
        return pd.Series("<blank>", index=df.index, dtype="object")
...
def inside_range(values: pd.Series, variable: str) -> pd.Series:
    """
    Boolean mask: valor numérico dentro del rango amplio.
    """
...
def classify_quality_masks(
    *,
    ...
    suspect:
        Se conserva si el valor físico es plausible, pero queda trazado como
        kept_raw_suspect_qc o rescaled_*_suspect_qc.
    """
    ...
    # General flags: keep physically plausible values, but mark as suspect.
    suspect |= qc.isin(GENERAL_REVIEW_FLAGS)

    # Blank and 9 are not treated as errors.
    strong_error &= ~qc.isin({"<blank>", "9"})
    suspect &= ~qc.isin({"<blank>", "9"})
...
def clean_variable(
    *,
    ...
    Las decisiones son strings trazables:
        non_numeric
        dropped_strong_quality_code
        kept_raw
        kept_raw_suspect_qc
        rescaled_divide_by_10
        rescaled_divide_by_10_suspect_qc
        rescaled_multiply_by_10
        rescaled_multiply_by_10_suspect_qc
    ...
    # 2. Mantener raw si está dentro de rango.
    raw_inside = numeric & ~strong_error & inside_range(raw, variable)

    mask_keep_raw = raw_inside & ~suspect
    mask_keep_raw_suspect = raw_inside & suspect

    clean.loc[mask_keep_raw] = raw.loc[mask_keep_raw]
    clean.loc[mask_keep_raw_suspect] = raw.loc[mask_keep_raw_suspect]

    decision.loc[mask_keep_raw] = "kept_raw"
    decision.loc[mask_keep_raw_suspect] = "kept_raw_suspect_qc"

    # 3. Reescalar temperature y dew point con /10 si raw está fuera de rango
    #    pero raw/10 entra al rango físico amplio.
    if variable in {"temperature", "dew_point_temperature"}:
        ...
        mask_rescale = rescale_ok & ~suspect
        mask_rescale_suspect = rescale_ok & suspect

        clean.loc[mask_rescale] = candidate.loc[mask_rescale]
        clean.loc[mask_rescale_suspect] = candidate.loc[mask_rescale_suspect]

        decision.loc[mask_rescale] = "rescaled_divide_by_10"
        decision.loc[mask_rescale_suspect] = "rescaled_divide_by_10_suspect_qc"

    # 4. Reescalar presión con *10 si raw está fuera de rango
    #    pero raw*10 entra al rango físico amplio.
    if variable == "station_level_pressure":
        ...
        decision.loc[mask_rescale] = "rescaled_multiply_by_10"
        decision.loc[mask_rescale_suspect] = "rescaled_multiply_by_10_suspect_qc"

    # 5. Todo lo numérico restante que no se limpió queda fuera de rango.
    unresolved = numeric & clean.isna() & ~strong_error
    ...
    for variable in CORE_VARIABLES:
        ...
        rows.append(
            CleaningRuleRow(
                ...
                quality_rule="Drop strong QC errors; keep suspect QC only if physically plausible and trace decision.",
            )
        )
...
def aggregate_hourly(work: pd.DataFrame) -> pd.DataFrame:
    ...
    Para variables de estado: promedio horario.
    Para precipitación: suma de los valores limpios dentro de la hora.
    """
    ...
    for variable in CORE_VARIABLES:
        ...
        if HOURLY_AGG_METHOD[variable] == "mean":
            agg = grouped[clean_col].mean().rename(out_col).reset_index()
        elif HOURLY_AGG_METHOD[variable] == "sum":
            agg = grouped[clean_col].sum(min_count=1).rename(out_col).reset_index()
        else:
            raise ValueError(f"Unknown aggregation method for {variable}")
...
def build_station_year_summary(hourly: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_dict = {
        "datetime_hour": "nunique",
        "n_raw_records_in_hour": "sum",
    }
